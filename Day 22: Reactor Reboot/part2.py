...
class Cuboid:
    ...
    def __repr__(self):
        return f'Size: {self.size}, ({self.x_min}, {self.x_max}), ({self.y_min}, {self.y_max}), ({self.z_min}, {self.z_max})'
        ...
        for x0, x1 in zip(xs[:-1], xs[1:]):
            for y0, y1 in zip(ys[:-1], ys[1:]):
                for z0, z1 in zip(zs[:-1], zs[1:]):
                    ...
                    if self.contains(cuboid) and not other.intersects(cuboid):
                        ret.append(cuboid)
        return ret
...
class Step(Cuboid):
    ...
    @classmethod
    def create(cls, s: str) -> Step:
        x_min, x_max, y_min, y_max, z_min, z_max = map(int, re.findall(r'-?\d+', s))
        return cls(x_min, x_max + 1, y_min, y_max + 1, z_min, z_max + 1, s[:2] == 'on')

    def __repr__(self):
        return f'{"On" if self.on else "Off"}, {super().__repr__()}'


def compute(s: str) -> int:
    steps = [Step.create(line) for line in s.splitlines()]
    cuboids: list[Cuboid] = []
    ...
    for step in steps:
        cuboids = [
            part
            for cube in cuboids
            for part in cube - step
        ]
        if step.on:
            cuboids.append(step)

    return sum(cube.size for cube in cuboids)
